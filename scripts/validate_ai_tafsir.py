...
def pass_coverage(counts: dict[int, int], surahs: list[int]) -> list[str]:
    errors: list[str] = []
    for surah in surahs:
...
def pass_length(surahs: list[int]) -> list[str]:
    errors: list[str] = []
    for surah in surahs:
        path = AI_DIR / f"surah_{surah}.json"
        if not path.exists():
...
def pass_structure(surahs: list[int]) -> list[str]:
    errors: list[str] = []
    for surah in surahs:
        path = AI_DIR / f"surah_{surah}.json"
        if not path.exists():
        ...
        for key, text in ayahs.items():
            if not REQUIRED_SECTION.search(text):
                errors.append(f"PASS 3 structure: {surah}:{key} missing 'What this ayah teaches'")
    return errors


def pass_quality(surahs: list[int]) -> list[str]:
    errors: list[str] = []
    for surah in surahs:
        path = AI_DIR / f"surah_{surah}.json"
        if not path.exists():
...
def pass_substance(surahs: list[int]) -> list[str]:
    errors: list[str] = []
    for surah in surahs:
        path = AI_DIR / f"surah_{surah}.json"
        if not path.exists():
...
def main() -> int:
    ...
    for surah in surahs:
        path = AI_DIR / f"surah_{surah}.json"
        if path.exists():
            total_ayahs += len(json.loads(path.read_text(encoding="utf-8")).get("ayahs", {}))

    print(f"Validating {total_ayahs} AI tafsir entries ({n_passes} passes)...\n")

    for i in range(n_passes):
        name, fn = PASSES[i]
        if name == "Coverage":
            errs = fn(counts, surahs)
        else:
            errs = fn(surahs)
